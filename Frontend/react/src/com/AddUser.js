...
import '../App.css';
export default function AddUser({ title, flag, setFlag, nav }) {
    ...
    const handleSubmit = () => {
        ...
        if(!image || !formValue.name){
            alert("יש להזין שם ותמונה 🤨")
            return
        }
    ...
    async function send(file) {
        ...
        try {
            const res = await Axios.post(`http://127.0.0.1:5000/addUser`, formData).then(result => {
                setRes(result.data)
                if (result.data)
        ...
        <div className="remove back" style={{backgroundImage: `url(./תמונה1.png)`}} >
            {!showMessage &&<Form ref={formRef} onChange={setFormValue} onCheck={setFormError} formValue={formValue} model={model}>
                <Form.Group controlId="name-2">
                    <Form.Control required checkAsync name="name" placeholder="שם משתמש" />
                </Form.Group>
                <Form.Group>
                ...
                <Form.Group>
                    <ButtonToolbar>
                        <Button appearance="primary" onClick={handleSubmit}>הוסף</Button>
                    </ButtonToolbar>
                </Form.Group>
            </Form>}
